							...
							?>
								{
									...
									backgroundColor: [
										<?php
														$query_calon_warna = "SELECT * FROM tbl_calon Where id_sekolah LIKE '$id_skl'";
														$sql_calon_warna = $pdo->prepare($query_calon_warna);
														$sql_calon_warna->execute();
														$jml_calon_warna = $sql_calon_warna->rowCount();
	
														while ($warna_calon = $sql_calon_warna->fetch()) {
															if ($kandidat_grafik['id_pil']==$warna_calon['id_pil']) {
																$rndcolor = rndRGBColorCode(); #function call
																?>
																'<?php echo $rndcolor; ?>'
																<?php
...
<div class="panel panel-default">
	<div class="panel-heading">
				<h3><strong>Data Kelas Memilih</strong></h3>
			</div>
				<div class="panel-body">
					<div class="row">
						<div class="col-md-12" >
